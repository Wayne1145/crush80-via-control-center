...
本项目是面向 **Crush 80 RGB 旗舰版**的本地中文 WebHID 控制工具。它只依据随项目保存的两份官方 VIA JSON，并只发送标准 VIA 协议中用于读取/配置的命令。

> **硬件验收状态：待验收。** 本项目已完成构建、模拟 HID 单元测试和浏览器 UI 审查，但这台开发环境无法接触 Wayne 的实体键盘。首次实体操作必须按本文的分阶段验收流程执行。
