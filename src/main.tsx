...
import {layoutBounds, normalizedKeyRect} from './via/layout-bounds';
...
function Keymap({connection,draft,selectedLayer,setSelectedLayer,selectedKey,setSelectedKey,selected,selectedCode,options,updateSelectedKey}:{connection:Connection;draft:DeviceState;selectedLayer:number;setSelectedLayer:(value:number)=>void;selectedKey:string;setSelectedKey:(value:string)=>void;selected:KeyboardKey;selectedCode:number;options:KeycodeOption[];updateSelectedKey:(value:number)=>void}) { const categoryOrder = ['全部','输入','媒体','层','设备','宏','灯光','鼠标','特殊']; const categories = categoryOrder.filter(item => item === '全部' || options.some(option => option.category === item)); const [category,setCategory] = useState('全部'); const [query,setQuery] = useState(''); const normalizedQuery = query.trim().toLocaleLowerCase(); const visible = options.filter(option => (category === '全部' || option.category === category) && (!normalizedQuery || option.label.toLocaleLowerCase().includes(normalizedQuery) || option.code.toLocaleLowerCase().includes(normalizedQuery))); return <><PageTitle eyebrow="键位设置" title="映射你的工作方式。"><p>键位位置严格读取官方 VIA JSON；每一枚可写键以 <code>Layer:Row:Column</code> 唯一标识。</p></PageTitle><div className="layer-bar">{Array.from({length: connection.layers}, (_, index) => <button key={index} className={selectedLayer === index ? 'active' : ''} onClick={() => setSelectedLayer(index)}>Layer {index}</button>)}</div><section className="keymap-grid"><div className="keymap-canvas"><OfficialKeyboard keys={connection.keys} selected={selectedKey} keycodes={draft.layers[selectedLayer]} custom={connection.definition.customKeycodes} macroCount={draft.macroCount} protocol={connection.protocol} onSelect={setSelectedKey}/><p>当前选择：矩阵 {selected.row},{selected.col} · 原始 keycode 显示在右侧。ISO/ANSI 兼容重复坐标已去重，不会生成重复写入。</p></div><aside className="key-inspector"><p className="eyebrow">当前选择</p><h2>{labelForKeycode(selectedCode, connection.definition.customKeycodes, draft.macroCount, connection.protocol)}</h2><small>Layer {selectedLayer} · Row {selected.row} · Column {selected.col} · 0x{selectedCode.toString(16).toUpperCase().padStart(4,'0')}</small><div className="keycode-search"><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索键名或 QMK 键码，如 Enter / KC_ENT" aria-label="搜索可分配键码"/>{query && <button type="button" onClick={() => setQuery('')}>清除</button>}</div><div className="key-categories">{categories.map(item => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="key-options">{visible.length ? visible.map(option => <button key={option.code} className={selectedCode === option.value ? 'active' : ''} onClick={() => updateSelectedKey(option.value)}><b>{option.label}</b><small>{option.code}</small></button>) : <p>当前类别没有受官方定义允许的可选项。</p>}</div></aside></section></>; }
function OfficialKeyboard({keys,selected,keycodes,custom,macroCount,protocol,onSelect}:{keys:KeyboardKey[];selected:string;keycodes:number[];custom:typeof usbDefinition.customKeycodes;macroCount:number;protocol:number;onSelect:(id:string)=>void}) { const bounds = layoutBounds(keys); return <div className="official-keyboard" style={{'--layout-width': String(bounds.width), '--layout-height': String(bounds.height)} as React.CSSProperties}>{keys.map(key => { const rect = normalizedKeyRect(key, bounds); const label = labelForKeycode(keycodes[key.row * 16 + key.col], custom, macroCount, protocol); return <button key={key.id} title={`${key.id} · ${label}`} className={selected === key.id ? 'selected' : ''} onClick={() => onSelect(key.id)} style={{left: `${rect.left}%`, top: `${rect.top}%`, width: `${rect.width}%`, height: `${rect.height}%`, '--key-color': key.color} as React.CSSProperties}><span>{label}</span></button>; })}</div>; }
